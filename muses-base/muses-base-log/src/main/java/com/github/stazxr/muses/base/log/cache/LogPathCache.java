    ...
    public static boolean allowedLogWithCache(String path, List<String> excludeList) {
        ...
        for (String excludePath : excludeList) {
            if (PATH_MATCHER.match(excludePath, path)) {
                log.info(MarkerConstants.MONITOR_MARKER, "log control path cache in exclude map: {}", path);
        ...
        log.info(MarkerConstants.MONITOR_MARKER, "log control path cache in include map: {}", path);
        return true;
    }
